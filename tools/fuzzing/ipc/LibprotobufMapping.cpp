...
template<typename T>
std::string LibprotobufMapping::ReadSerializedParam(IPC::MessageReader* reader, UniquePtr<IPC::Message>& msg) {
  ...
  return std::string(dumpBuffer.begin(), dumpBuffer.length());
}
